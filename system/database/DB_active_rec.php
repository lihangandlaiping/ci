...
class CI_DB_active_record extends CI_DB_driver {
	...
	public function join($table, $cond, $type = '')
	{
		...
		if (preg_match('/([\w\.]+)([\W\s]+)(.+)/', $cond, $match))
		{
           ...
           $match[1]=str_replace($this->dbprefix,'',$match[1]);
            $match[3]=str_replace($this->dbprefix,'',$match[3]);
	...
	protected function _like($field, $match = '', $type = 'AND ', $side = 'both', $not = '')
	{
		...
		foreach ($field as $k => $v)
		{
			...
			$this->ar_like[] = str_replace($this->dbprefix,'',$like_statement);
			if ($this->ar_caching === TRUE)
			{
		...
		else
		{
			...
			else
			{
				...
				foreach ($this->ar_select as $key => $val)
				{
					...
					$this->ar_select[$key] = str_replace($this->dbprefix,'',$this->_protect_identifiers($val, FALSE, $no_escape));
				}
