...
class Form
{
    ...
    function initEdit($name, $content = '')
    {
        ...
        $html .= '<textarea class="" aria-required="true" id="' . $name . '_id" name="' . $name . '">' . $content . '</textarea>
...
<script type="text/javascript">
     ...
     * 复选框
     * @param $name
     * @param array $val array(array('1','看是'),array('2','看是'))
     * @param string $value
     * @return string
     */
    function checkbox($name, $val = array(array()), $value = '')
    {
        ...
        if(is_array($val[0])){
            ...
            foreach ($val as $k=>$v) {
                if (!$k || !$v) continue;
                $ischecked = in_array($k,$value) ? 'checked=checked' : '';
                $ischeckclass = in_array($k,$value) ? 'checked' : '';
                $html .= '<label class="checkbox-inline i-checks">
                                        <div class="icheckbox_square-green " style="position: relative;"><input  ' . $ischecked . ' type="checkbox" name="' . $name . '[]" value="' . $k . '" style="position: absolute; opacity: 0;"><ins class="iCheck-helper" style="position: absolute; top: 0%; left: 0%; display: block; width: 100%; height: 100%; margin: 0px; padding: 0px; border: 0px; opacity: 0; background: rgb(255, 255, 255);"></ins></div>' . $v . '</label>';
            }
        }
    ...
    function selectform($name, $val = array(array()), $value = '',$iswidth='')
    {
        ...
        $html .= '<div class="input-group">
                                        <select name="'.$name.'" data-placeholder="请选择..." class="chosen-select form-control"';
    ...
    function selectToform($name, $val = array(), $value = '',$iswidth='')
    {
        $html = '';
        if (!$this->is_initselect) $html .= $this->importSelect();
        $html .= '<div class="input-group">
        ...
        $option = '';
        foreach ($val as $key => $row) {
            $is_check=($value==$row || $key==$value)?'selected=selected':'';
            $option .= '<option '.$is_check.' value="' . $key . '" hassubinfo="true">' . $row . '</option>';
        }
        $html = sprintf($html, $option);
        // var_dump($html);exit;
        return $html;
    ...
    function linkPage($row,$vals,$value)
    {
        ...
        $html='<style>#'.$row['field'].' select{width:220px; float:left;}</style><div id="'.$row['field'].'"></div><script>$(function(){
        ...
        foreach ($fieldList as $row) {
            ...
            switch ($row['type']) {
                ...
                case 'date':
                    ...
                    break;
                case 'datetime':
                    $value[$row['field']]=$value[$row['field']]?date('Y-m-d H:i:s',$value[$row['field']]):'';
                ...
                case 'tablefield':
                    ...
                    $where=array();
                    if(!empty($newarray['search_where'])){
                        $newarray['search_where']=explode(',',$newarray['search_where']);
                        foreach ($newarray['search_where'] as $v){
                            $where_info=array_filter(preg_split('/\>|\<|\>\=|\<\=|\=/s',$v));
                            if(!isset($where_info[0])||!isset($where_info[1]))continue;
                            $ty=str_replace($where_info[1],'',str_replace($where_info[0],'',$v));
                            $where[trim($where_info[0]).' '.$ty]=$where_info[1];
                        }
                    }
                    $CI =& get_instance();
                    $list = $CI->mastermodel->inIt($newarray['db_table'])->filed("{$newarray['primary_key']},{$newarray['search_field']}")->getList($where);
                    $valus = array();
