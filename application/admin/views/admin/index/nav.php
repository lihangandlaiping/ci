...
<nav class="navbar-default navbar-static-side" role="navigation">
    ...
    <div class="">
        <ul class="nav" id="side-menu">
            ...
            <?php $level=isset($level)?$level:0;?>
            <?php foreach($menulist as $key=>$row):?>
            <?php if($admin_user['type']==1||in_array($row['id'],$menus)):?>
            <li > <a class="<?php echo $row['url']?'J_menuItem':'';?>" href="<?php echo $row['url']?site_url($row['url']):'#';?>"> <i class="<?php echo $row['log'];?>"></i> <span class="nav-label"><?php echo $row['title'];?></span></a>
